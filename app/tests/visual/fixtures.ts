// Shared fixtures for the result-layout and graph-viewport visual specs, so
// both exercise the same run/5D-graph shape instead of drifting apart.
export const runId = "visual-layout-run";
export const now = Date.parse("2026-07-06T17:32:47Z");
...
/** Registers the 5D-graph route mock and seeds the visual-test result hook. */
export async function seedVisualResult(page: import("@playwright/test").Page) {
  ...
  await page.addInitScript((entry) => {
    ...
    window.localStorage.setItem("causalops:history:v1", JSON.stringify([entry]));
  }, historyEntry);
}
